...
export function getSchemaVariables(schema, expressionLanguage = new FeelExpressionLanguage(null)) {
  ...
  const variables = schema.components.reduce((variables, component) => {
    ...
    if (conditional && conditional.hide) {
      ...
      // cut off initial '='
      const conditionVariables = expressionLanguage.getVariableNames(conditional.hide.slice(1), { type: 'unaryTest' });
    ...
    EXPRESSION_PROPERTIES.forEach((prop) => {
      ...
      if (property && expressionLanguage.isExpression(property)) {
        ...
        // cut off initial '='
        const expressionVariables = expressionLanguage.getVariableNames(property.slice(1), { type: 'expression' });
